...
from gui_eye_tracker_app import (
    RaycastEyeTrackerApp,
...
class TrackingWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"TrackingWorker Error: Model file '{model_path}' not found. Cannot start tracking."
            )
            self._is_running = False
        except Exception as e:
            print(
                f"TrackingWorker Error: Error loading model: {e}. Cannot start tracking."
            )
            self._is_running = False
        ...
        print(
            "TrackingWorker: Real-time loop in background. Press 'q' on OpenCV window or use GUI to quit."
        )
        ...
        while self._is_running:  # Loop based on internal flag
            ...
            if results.multi_face_landmarks:
                ...
                if (
                    gesture_click_map.get("Long Blink")
                    and both_eyes_closed_frames == LONG_BLINK_FRAMES
                    and not long_blink_triggered
                ):
                    pyautogui.click(button=gesture_click_map["Long Blink"])
                    long_blink_triggered = True
                    cv2.putText(
                        frame,
                        f"{gesture_click_map['Long Blink'].capitalize()} Click (Long Blink)",
                        (10, 90),
                ...
                if both_eyes_closed_frames == 0:
                    long_blink_triggered = False

                if (
                    gesture_click_map.get("Left Wink")
                    and left_eye_closed_frames == BLINK_FRAMES
                    and right_eye_closed_frames == 0
                    and not left_wink_triggered
                ):
                    pyautogui.click(button=gesture_click_map["Left Wink"])
                    left_wink_triggered = True
                    cv2.putText(
                        frame,
                        f"{gesture_click_map['Left Wink'].capitalize()} Click (Left Wink)",
                        (10, 120),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (255, 255, 0),
                ...
                if left_eye_closed_frames == 0 or right_eye_closed_frames > 0:
                    left_wink_triggered = False

                if (
                    gesture_click_map.get("Right Wink")
                    and right_eye_closed_frames == BLINK_FRAMES
                    and left_eye_closed_frames == 0
                    and not right_wink_triggered
                ):
                    pyautogui.click(button=gesture_click_map["Right Wink"])
                    right_wink_triggered = True
                    cv2.putText(
                        frame,
                        f"{gesture_click_map['Right Wink'].capitalize()} Click (Right Wink)",
                        (10, 150),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (0, 255, 255),
                ...
                if right_eye_closed_frames == 0 or left_eye_closed_frames > 0:
                    right_wink_triggered = False
                ...
                input_tensor = (
                    torch.tensor(eye_input, dtype=torch.float32).unsqueeze(0).to(device)
                )
            ...
            if cv2.waitKey(1) & 0xFF == ord("q"):
                print(
                    "TrackingWorker: 'q' pressed on OpenCV window. Stopping tracking."
                )
                self._is_running = False
                break
...
def _stop_tracking_handler():
    ...
    if _tracking_thread and _tracking_thread.isRunning():
        ...
        if _tracking_thread.isRunning():
            print(
                "Main: Warning: Tracking thread did not terminate gracefully. Terminating forcefully."
            )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Generated dummy arrow_down.png for QComboBox. You can replace this with a custom icon."
        )
