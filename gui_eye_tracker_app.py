...
from eye_tracker_utils import (
    get_eye_bbox,
    crop_and_preprocess_eye,
    generate_grid,
    is_face_stable,
    draw_calibration_dot,
    eye_aspect_ratio,
    find_working_camera,
    LEFT_EYE_EAR_IDXS,
    RIGHT_EYE_EAR_IDXS,
...
class RaycastEyeTrackerApp(QWidget):
    ...
    def __init__(self, screen_w, screen_h):
        ...
        if not self.cap.isOpened():
            print(
                f"Error: Could not open camera at index {self.cam_index}. Make sure it's not in use."
            )
        ...
        self.calibration_video_timer.timeout.connect(
            self._update_calibration_video_feed
        )

        self.calibration_points = generate_grid(
            rows=5, cols=7, screen_w=self.screen_w, screen_h=self.screen_h
        )
        ...
        self.x_min_calib, self.y_min_calib, self.x_max_calib, self.y_max_calib = (
            0.0,
            0.0,
            0.0,
            0.0,
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if self.windowState() != Qt.WindowMaximized:
            ...
            if on_top and on_left:
                self._resize_direction = Qt.SizeFDiagCursor
            elif on_top and on_right:
                self._resize_direction = Qt.SizeBDiagCursor
            elif on_bottom and on_left:
                self._resize_direction = Qt.SizeBDiagCursor
            elif on_bottom and on_right:
                self._resize_direction = Qt.SizeFDiagCursor
            elif on_left:
                self._resize_direction = Qt.SizeHorCursor
            elif on_right:
                self._resize_direction = Qt.SizeHorCursor
            elif on_top:
                self._resize_direction = Qt.SizeVerCursor
            elif on_bottom:
                self._resize_direction = Qt.SizeVerCursor
            ...
            if (
                self._resize_direction != Qt.ArrowCursor
                and self._resize_direction != Qt.SizeAllCursor
            ):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self._dragging_window and event.buttons() == Qt.LeftButton:
            ...
            if self._resize_direction == Qt.SizeHorCursor:
                if pos.x() <= self._resize_border_width:
                    new_x += delta_x
                    new_w -= delta_x
                else:
                    new_w += delta_x
            elif self._resize_direction == Qt.SizeVerCursor:
                if pos.y() <= self._resize_border_width:
                    new_y += delta_y
                    new_h -= delta_y
                else:
                    new_h += delta_y
                ...
                if (
                    pos.x() <= self._resize_border_width
                    and pos.y() <= self._resize_border_width
                ):
                    new_x += delta_x
                    new_w -= delta_x
                    new_y += delta_y
                ...
                if (
                    pos.x() >= rect.width() - self._resize_border_width
                    and pos.y() <= self._resize_border_width
                ):
                    new_w += delta_x
                    new_y += delta_y
    ...
    def _show_welcome_screen(self):
        ...
        if self.cap and self.cap.isOpened():
            ...
            self.welcome_status_label.setText(
                "Choose a profile or create a new one to begin."
            )
            ...
            self.welcome_status_label.setText(
                f"Camera at index {self.cam_index} not found. Please check connection.\nCannot create or load profiles without camera."
            )
    ...
    def _on_create_new_profile(self):
        if not self.cap or not self.cap.isOpened():
            QMessageBox.warning(
                self,
                "Camera Not Ready",
                "Cannot create a new profile without a connected camera. Please ensure your camera is working.",
            )
            return
        ...
        while True:
            ...
            if not profile_name.strip():
                QMessageBox.warning(
                    self, "Invalid Name", "Profile name cannot be empty."
                )
                continue
            ...
            if not all(c.isalnum() or c == "_" for c in sanitized_name):
                QMessageBox.warning(
                    self,
                    "Invalid Name",
                    "Profile name must be alphanumeric with optional spaces or underscores.",
                )
                continue
            ...
            if os.path.exists(self.profile_dir):
                QMessageBox.warning(
                    self,
                    "Profile Exists",
                    f"Profile '{profile_name}' already exists. Please choose a different name or load the existing profile.",
                )
            else:
                try:
        ...
        self.calibration_data_path = os.path.join(
            self.profile_dir, "calibration_data.json"
        )
        ...
        QMessageBox.information(
            self,
            "Profile Created",
            f"Profile '{self.profile_name}' created. Proceeding to calibration setup.",
        )
    ...
    def _on_load_profile(self):
        ...
        available_profiles = [
            d
            for d in os.listdir(self._profiles_base_dir)
            if os.path.isdir(os.path.join(self._profiles_base_dir, d))
        ]
        ...
        profile_name, ok = QInputDialog.getItem(
            self, "Load Profile", "Select a profile:", available_profiles, 0, False
        )
        ...
        self.mappings_path = os.path.join(
            self.profile_dir, "gesture_mappings.json"
        )  # Path for mappings
        ...
        try:
            ...
            if os.path.exists(self.mappings_path):
                ...
                QMessageBox.information(
                    self,
                    "Mappings",
                    "No custom mappings found for this profile. Default mappings loaded.",
                )

            QMessageBox.information(
                self,
                "Profile Loaded",
                f"Profile '{self.profile_name}' loaded successfully. You can now start tracking or re-calibrate.",
            )
            self._show_setup_mode()
            ...
            QMessageBox.critical(
                self,
                "Load Error",
                "Profile data (model.pt or calibration.json) not found for this profile. It might be incomplete.",
            )
    ...
    def _on_save_mappings(self):
        ...
        if not self.profile_name:
            QMessageBox.warning(
                self,
                "No Profile",
                "Please create or load a profile before saving mappings.",
            )
            return
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Mappings Saved",
                f"Gesture mappings saved for profile '{self.profile_name}'.",
            )
        except Exception as e:
    ...
    def _on_delete_selected_profile(self):
        ...
        if not selected_items:
            QMessageBox.warning(
                self, "No Selection", "Please select a profile to delete."
            )
            return

        selected_item = selected_items[0]
        profile_dir_name_to_delete = selected_item.data(
            Qt.UserRole
        ...
        if reply == QMessageBox.Yes:
            profile_path = os.path.join(
                self._profiles_base_dir, profile_dir_name_to_delete
            )
            try:
                ...
                QMessageBox.critical(
                    self,
                    "Delete Error",
                    f"Could not delete profile '{readable_name}': {e}",
                )
    ...
    def _start_tracking_from_setup(self):
        if (
            ...
            or not self.model
            or not (self.x_max_calib and self.y_max_calib)
        ):
            QMessageBox.warning(
                self,
                "No Profile/Data",
                "Please ensure a profile is loaded with valid calibration data before starting tracking.",
            )
            return
        ...
        if not self.cap or not self.cap.isOpened():
            QMessageBox.critical(
                self,
                "Camera Not Ready",
                "Camera is not open. Please ensure your camera is connected and working before starting tracking.",
            )
            return
    ...
    def gesture_comboboxes_to_map(self):
        ...
        for gesture in gesture_options:
            ...
            if val == "Left Click":
                mapping[gesture] = "left"
            elif val == "Right Click":
                mapping[gesture] = "right"
            else:
    ...
    def setup_ui(self):
        ...
        self.main_layout.addWidget(
            window_controls_container, alignment=Qt.AlignLeft | Qt.AlignTop
        )
        ...
        self.welcome_status_label = QLabel(
            "Choose a profile or create a new one to begin."
        )
        ...
        setup_layout_content.addWidget(
            self.video_label_setup_mode, alignment=Qt.AlignCenter
        )

        self.status_label_setup_mode = QLabel(
        ...
        self.profile_list_widget.itemSelectionChanged.connect(
            self._on_profile_list_selection_changed
        )
        ...
        self.delete_selected_profile_btn.setEnabled(False)
        self.delete_selected_profile_btn.clicked.connect(
        ...
        profile_manage_layout.addWidget(
            profile_manage_back_btn, alignment=Qt.AlignCenter
        )
        ...
        self.fullscreen_video_label.setSizePolicy(
    ...
    def resizeEvent(self, event):
        ...
        if self.stacked_widget.currentWidget() == self.calibration_view:
            ...
            self.capture_btn_overlay.setGeometry(
                padding, current_height - btn_height - padding, btn_width, btn_height
            )
            self.capture_btn_overlay.raise_()

            stop_btn_x = padding + btn_width + 15
            self.stop_btn_overlay.setGeometry(
                stop_btn_x, current_height - btn_height - padding, btn_width, btn_height
            )
            self.stop_btn_overlay.raise_()

            self.finish_btn_overlay.setGeometry(
                current_width - btn_width - padding,
                current_height - btn_height - padding,
                btn_width,
                btn_height,
            )
    ...
    def _update_calibration_video_feed(self):
        ...
        if frame is not None:
            if (
                ...
                and self.calibration_state["current_point_idx"]
                < self.calibration_state["total_points"]
            ):
                cx, cy = self.calibration_points[
                    self.calibration_state["current_point_idx"]
                ]
                ...
                bytes_per_line = ch * w
                convert_to_qt_format = QImage(
                    rgb_image_with_dot.data, w, h, bytes_per_line, QImage.Format_RGB888
                )
    ...
    def start_calibration(self):
        if not self.profile_name:
            QMessageBox.warning(
                self,
                "No Profile Selected",
                "Please select or create a profile before starting calibration.",
            )
            return
        ...
        if not self.cap or not self.cap.isOpened():
            QMessageBox.critical(
                self,
                "Camera Error",
                "Camera is not open. Please check connection and permissions, then restart the application.",
            )
            return
        ...
        self.capture_btn_overlay.show()
        self.capture_btn_overlay.raise_()
        self.finish_btn_overlay.show()
        self.finish_btn_overlay.raise_()
        self.stop_btn_overlay.show()
        self.stop_btn_overlay.raise_()
        self.calibration_status_overlay.show()
        self.calibration_status_overlay.raise_()
        self.training_progress_bar.hide()
    ...
    def capture_sample(self):
        ...
        if not ret:
            self.calibration_status_overlay.setText(
                "Failed to capture frame for sample. Try again."
            )
            return
        ...
        if results.multi_face_landmarks:
            ...
            if not is_face_stable(self.iris_history):
                self.calibration_status_overlay.setText(
                    "Face unstable. Keep your head still and try capturing again."
                )
                return
            ...
            if (
                ...
                self.stop_btn_overlay.hide()
                self.calibration_status_overlay.setText(
                    "Calibration data collected. Click 'Finish & Start Tracking'."
                )
                self.calibration_video_timer.stop()
                self.cap.release()
            ...
            self.calibration_status_overlay.setText(
                "Face not detected. Adjust camera or your position and try again."
            )

    def finish_and_start_tracking(self):
        if (
            len(self.calibration_data)
            < self.samples_per_point * self.calibration_state["total_points"]
        ):
            QMessageBox.warning(
                self,
                "Incomplete Calibration",
                "Not enough calibration data. Please capture all samples for all points before finishing.",
            )
            return
        ...
        X = torch.tensor(np.array(self.calibration_data), dtype=torch.float32).to(
            self.device
        )
        y = torch.tensor(np.array(self.calibration_labels), dtype=torch.float32).to(
            self.device
        )
        ...
        self.calibration_status_overlay.setText(
            "Setup complete. Starting eye tracking!"
        )
